function check() {
  ...
  function showWeather(weather, city, arr) {
    ...
    let forecastData = arr.list.slice(0, 7); // Grab first 7 days
    for (let i = 0; i < forecastData.length; i++) {
      document.querySelectorAll(".tempMax")[i].innerText = forecastData[i].main.temp_max + "°";
      document.querySelectorAll(".tempMin")[i].innerText = forecastData[i].main.temp_min + "°";
    }
  }
  ...
  async function getWeatherByCoordinates(lat, lon) {
    try {
      let response = await fetch(
        `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=5b2fe86acda81f8761fcf8f70357d4ee&units=metric`
      );
  
      let data = await response.json();
  
      let responseData2 = await fetch(
        `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=5b2fe86acda81f8761fcf8f70357d4ee`
      );
  
      let arr = await responseData2.json();
  
      showWeather(data, "Your Location", arr);
    } catch (err) {
      console.log(err);
    }
  }
